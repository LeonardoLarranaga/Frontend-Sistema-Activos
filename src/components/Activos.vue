        ...
        <v-dialog v-model="dialog" max-width="500px">
          ...
          <v-card width="120%" style="transform: translateX(-20px);">
            <v-card-title>
              ...
              <span class="text-h5 font-weight-bold"> {{ this.editedIndex === -1 ? "Nuevo Activo" : "Editar Activo" }}
              </span>
            </v-card-title>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6">
                    <v-select :item-props="this.ubicacionProps" v-model="this.ubicacionSelected" clearable
                      label="Ubicación" :items="this.ubicaciones" prepend-inner-icon="mdi-archive-marker" />
                  </v-col>

                  <v-col cols="12" sm="6">
                    <v-select :item-props="this.responsableProps" v-model="this.responsableSelected" clearable
                      label="Responsable" :items="this.responsables" prepend-inner-icon="mdi-account-tie" />
                  </v-col>

                  <v-col cols="12">
                    <v-select :item-props="this.tagProps" v-model="this.tagsSelected" clearable label="Tags"
                      :items="this.tags" chips multiple prepend-inner-icon="mdi-tag-multiple" />
                  </v-col>
  ...
  methods: {
    ...
    async createActivo() {
      try {
        ...
        const response = await axios.post("https://localhost:4000/activo/", this.editedItem, this.store.headers)
        const newActivo = response.data.activo
        this.activos.push(newActivo)

        this.tagsSelected.forEach((tag) => {
          axios.post(`https://localhost:4000/activo/${newActivo.id}/tag/${tag.id}`, {}, this.store.headers)
        })
    ...
    async editActivo() {
      try {
        ...
        const response = await axios.put(`https://localhost:4000/activo/${this.editedItem.id}`, this.editedItem, this.store.headers)

        await axios.delete(`https://localhost:4000/activo/${response.data.activo.id}/tag/`, this.store.headers)

        this.tagsSelected.forEach((tag) => {
          axios.post(`https://localhost:4000/activo/${response.data.activo.id}/tag/${tag.id}`, {}, this.store.headers)
        })
    ...
    async editItem(item) {
      try {
        ...
        const tags = (await axios.get(`https://localhost:4000/activo/${item.id}/tags`, this.store.headers)).data
        this.tagsSelected = []
    ...
    async deleteItemConfirm() {
      try {
        const response = await axios.delete(`https://localhost:4000/activo/${this.editedItem.id}`, this.store.headers)
    ...
    async showTagsDialog(item) {
      try {
        this.tagsSelected = (await axios.get(`https://localhost:4000/activo/${item.id}/tags`, this.store.headers)).data
